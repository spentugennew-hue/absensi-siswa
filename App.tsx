...
const Dashboard = ({ 
  ...
  // Prepare Bar Chart Data (Group by Minute for better granularity)
  ...
  todayRecords.forEach(r => {
    // time_str is "HH:mm:ss", we want "HH:mm"
    // Ensure we handle both "13:30:00" and "13.30.00" if data varies
    const cleanTime = r.time_str.replace(/\./g, ':');
    const timeKey = cleanTime.substring(0, 5);
    minuteData[timeKey] = (minuteData[timeKey] || 0) + 1;
  });

  // Sort by time and create array
  const barData = Object.keys(minuteData).sort().map(time => ({
    name: time,
    val: minuteData[time]
  }));
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
           <h2 className="text-2xl font-bold text-slate-800 dark:text-white">Dashboard Absensi</h2>
           <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">
             {currentTime.toLocaleDateString('id-ID', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}
           </p>
        </div>
        <div className="flex gap-3 items-center">
            <div className="bg-white dark:bg-[#27293d] px-4 py-2 rounded-lg shadow-sm border border-slate-200 dark:border-slate-700 flex items-center gap-2 text-slate-800 dark:text-white font-mono font-bold text-lg">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-80">
        {/* Bar Chart */}
        <Card className="lg:col-span-2 flex flex-col" title="Grafik Absensi (Per Menit)">
          <div className="flex-1 w-full min-h-0">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={barData}>
                ...
                />
                <Bar dataKey="val" name="Jumlah Siswa" fill="#4f46e5" radius={[4, 4, 0, 0]} barSize={20} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </Card>
        ...
        {/* Pie Chart */}
        <Card className="flex flex-col" title="Persentase Hari Ini">
...
const AttendanceScanner = ({ 
  ...
  const processScan = () => {
    ...
    const now = new Date();
    // Normalize time string to HH:mm:ss with colons
    ...
    if (student) {
      ...
      if (mode === 'masuk') {
        // Dynamic Check-in Time Logic
        const [limitHour, limitMinute] = settings.checkInTime.split(':').map(Number);
    ...
    <div className="flex flex-col w-full max-w-6xl mx-auto h-full space-y-8 pb-10">
      <div className="flex flex-col items-center pt-4">
        ...
        <div className="flex items-center gap-2 text-slate-500">
          <Clock size={16} />
          <span className="text-sm font-medium">Batas Terlambat: {settings.checkInTime}</span>
        </div>
      </div>
...
Terima kasih.`;
  ...
  const handleScan = (student: Student, status: AttendanceRecord['status'], description?: string) => {
    ...
    const now = new Date();
    // Normalize time string (HH:mm:ss)
    const timeStr = now.toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit', second: '2-digit' }).replace(/\./g, ':');
      ...
      <div className="flex-1 flex flex-col overflow-hidden bg-slate-50 dark:bg-[#1e1e2f] transition-colors duration-300">
        ...
        <main className="flex-1 overflow-y-auto p-8 custom-scrollbar flex flex-col">
           ...
           <div className="flex-1">
               ...
               {activePage === Page.PRESENSI && <AttendanceScanner students={students} records={records} onScan={handleScan} initialMode={presensiMode} settings={appSettings} />}
